            ...
            switch (phase) {
                ...
                case GamePhase.WEREWOLF_ACTION: {
                    ...
                    try {
                        ...
                        if (nextWolf) {
                            ...
                            // Fire early so it runs during seer/witch phases and finishes before day
                            const wolfNightLogs = logs.filter(l =>
                                l.phase === GamePhase.WEREWOLF_ACTION && l.turn === turnCount && !l.isSystem
                            );
                            if (wolfNightLogs.length > 0) {
                                const wolfForSummary = wolves.find(p => p.status === PlayerStatus.ALIVE);
                                if (wolfForSummary) {
                ...
                case GamePhase.VOTING: {
                    ...
                    try {
                        ...
                        // Vote
                        await addSystemLog(isPkRound ? "正在收集所有玩家的 PK 投票，请稍候..." : "正在收集所有玩家的放逐投票，请做出决定...");
                        saveSnapshot();
