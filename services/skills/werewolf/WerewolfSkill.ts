...
## 核心判据：客观事实高于一切
1. **事实是检验真伪的唯一标准**：任何发言的价值，100% 取决于其陈述的事实（夜间死伤、真实刀口、查验结果、投票记录）是否客观属实、逻辑是否闭环。
2. **严禁以貌取人**：**绝对禁止**仅凭“发言长短、语气是否积极、态度是否敷衍”给好人扣狼标！简明扼要陈述客观事实的发言享有最高信任权重。
3. **凝聚好人共识**：当场上有玩家摆出铁证（如刀口矛盾、报验冲突）拆穿假预言家或锁定狼人时，好人阵营必须正面认同该事实并跟进共识，合力指认狼人。`.trim();
...
- 示例: { "speak": "警徽移交给9号，大家跟着他走。", "actionTarget": 9 }`
        ...
        if (phase === GamePhase.DAY_DISCUSSION || phase === GamePhase.LAST_WORDS || phase === GamePhase.DAY_ANNOUNCE) {
            ...
            return {
                ...
                constraints: `- **核心任务**：
  1. **事实核查与回应**：针对前序核心争论（尤其是预言家对跳、他人指出的刀口与逻辑矛盾），核实其是否符合客观事实，明确表态认同或反驳。
  2. **交代狼坑**：结合死伤、查验与站队，向全场清晰交代你认定的疑似狼坑组合。
- 格式: { "speak": "你的公开演讲/发言内容（控制在300字以内）", "summary": "15字以内的发言核心要点" }
- 示例: { "speak": "昨晚倒牌的是4号，5号报4号是金水完全不符合夜间事实，我认同前面玩家指出的矛盾，5号必是悍跳狼。我目前的狼坑锁定在5号以及刚才帮他冲票的8号。", "summary": "抓出5号逻辑漏洞标狼" }`
            };
        }
