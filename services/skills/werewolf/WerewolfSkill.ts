...
- **狼人阵营**：唯一目标是【掩盖夜间真相，制造混乱与误导，带领狼人团队获胜】。

## 高阶竞技思维（真相推演体系）
1. **双世界假设推演（主干框架）**：
   面对预言家对跳或重大分歧时，绝不凭感觉站边，而是用“假设法”对比验证两个平行假设（假设A：若X为真 vs 假设B：若Y为真），从以下三层维度检验哪个世界更自洽：
   - **事实维度**：夜间真实死伤、刀口位置、查验结果能不能严密对得上？
   - **动机与收益维度**：玩家的行为、查验和站队是否符合该阵营的常理收益？是否存在违背阵营利益的反常漏洞？
   - **狼坑闭环维度**：按该假设推演，全场剩下的狼坑能否完整排满且合情合理？
   对比两方，坚决站边矛盾更少、闭环更紧密的世界。
2. **置信度分层与狼坑闭环**：
...
4. **自然说话，严禁报菜名**：
   **严禁在公开发言中说出“双世界推演”、“置信度”等概念术语**！这些是你的内在思考逻辑，发言时要像真人高玩一样自然论证。
5. **铁证面前凝聚共识**：
...
- 示例: { "speak": "警徽移交给9号，大家跟着他走。", "actionTarget": 9 }`
        ...
        if (phase === GamePhase.DAY_DISCUSSION || phase === GamePhase.LAST_WORDS || phase === GamePhase.DAY_ANNOUNCE) {
            ...
            return {
                ...
                constraints: `- **核心任务**：
  1. **假设推演与站边**：针对对跳或核心争议，说明你站边的依据（结合事实对撞、行为动机与全场自洽度），并正面回应前人的事实证据。
  2. **明确狼坑闭环**：向全场清晰交代你认定的完整狼坑名单（如9人局排出3狼组合）。
- **自然说话**：必须像真人玩家一样交流，**严禁在发言中出现“双世界推演”、“置信度”等术语字样**，避免生硬套话。
- 格式: { "speak": "你的公开演讲/发言内容（控制在300字以内）", "summary": "15字以内的发言核心要点" }
- 示例: { "speak": "如果5号是真预言家，昨晚死的是4号，5号报4号是查杀根本对不上夜间刀口；而且5号视野里的狼坑严重排不开。反观2号的验人符合常理，我坚定站边2号真预言家。目前场上的狼坑锁定在：悍跳的5号、以及为他冲锋的8号和9号。", "summary": "推演事实与狼坑站边2号" }`
            };
        }
