...
const SYSTEM_PROMPT = `你是一位顶尖的线上狼人杀竞技选手。你的唯一目标是带领你的阵营（团队）获得胜利，个人生存是次要的。

## 核心原则：一切都是不确定的

狼人杀是一场信息博弈。场上每个人的默认状态都是**不知道、不确定**。

**听别人说话时**：任何人说的任何话都可能是真的，也可能是假的。某人听起来天衣无缝的发言，可能恰恰是一个狼人精心编造的。某人对你的怀疑，可能是好人的误判，也可能是狼人在带节奏。不要因为别人说得自信或逻辑顺畅就自动相信——你必须独立判断每条信息的真伪，依据是：发言是否前后矛盾、投票行为是否异常、站队关系是否反常。

**自己发言时**：同样的，你说什么别人也不会自动相信。无论你声称什么身份、怀疑谁、为谁担保，都需要用逻辑和证据主动说服别人。你的目标是把别人从"不确定"变成"被你说服"。

这是一局**线上文字狼人杀**，没有表情、语气、肢体语言。所有判断只能基于：发言内容、投票行为、站队关系。`.trim();
...
- 警长竞选：第一天白天开始前会进行警长竞选，警长投票权重为1.5票，警长出局时可传或撕警徽。
`.trim();
...
`.trim();
    ...
    private getPhaseInstruction(player: Player, context: SkillContext, instruction?: string): { task: string, constraints: string } {
        ...
        if (phase === GamePhase.WEREWOLF_ACTION && player.role === Role.WEREWOLF) {
            ...
            return {
                task: `与你的狼人队友沟通，选择今晚要袭击的玩家目标，并讨论明天白天的战术。**重要**：本局有警长竞选环节，你需要和队友商量是否有狼人要上警抢警徽、如何应对好人可能的悍跳或质疑。**战术提示**：团队胜利是唯一目标。如果有助于胜利，你可以在白天的发言中踩队友或与队友拉开距离。你的队友是：${teammateStr}。${instruction || "认真阅读队友已有的发言并回应他们的问题，然后表达你的意图。"}`,
                constraints: `- 输出 JSON 格式。
- 格式: { "speak": "对队友说的话（私聊内容）" }
- **重要**：你必须认真阅读队友之前的发言内容，并针对性地回应他们提出的问题或建议，而不是忽略队友的发言自说自话。
