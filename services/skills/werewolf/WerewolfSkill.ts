...
## 高阶竞技思维（真相推演体系）
1. **双世界假设推演（反事实验证）**：
   面对预言家对跳或核心分歧时，绝不凭感觉站边，而是用“假设法”对比验证：
   - 假设世界A（若X为真预言家）：夜间死伤能否解释？按其视角的狼坑在全场的站队与票型是否自洽？
   - 假设世界B（若Y为真预言家）：X为悍跳狼，谁在冲锋谁在掩护？全场狼坑能否闭环？
   - 对比哪个世界的逻辑矛盾更少、解释力更强，坚决站边更自洽的世界。
2. **置信度分层与狼坑闭环**：
...
- 示例: { "speak": "警徽移交给9号，大家跟着他走。", "actionTarget": 9 }`
        ...
        if (phase === GamePhase.DAY_DISCUSSION || phase === GamePhase.LAST_WORDS || phase === GamePhase.DAY_ANNOUNCE) {
            ...
            return {
                ...
                constraints: `- **核心任务**：
  1. **双世界推演与回应**：针对对跳或核心争议，简述你站边的逻辑（哪个假设世界更自洽），并正面回应前人指出的事实证据。
  2. **明确狼坑闭环**：向全场清晰交代你认定的完整狼坑名单（如9人局排出3狼组合）。
- 格式: { "speak": "你的公开演讲/发言内容（控制在300字以内）", "summary": "15字以内的发言核心要点" }
- 示例: { "speak": "若5号是真预言家，昨晚倒牌的4号怎么可能反向吃刀？假设5号为悍跳狼，逻辑立刻闭环。因此我站边2号真预言家，目前的完整狼坑锁定为：悍跳的5号、以及为他冲锋的8号和9号。", "summary": "双世界推演揭露5号并排狼坑" }`
            };
        }
