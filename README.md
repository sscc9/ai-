...
- **没有给笨模型打补丁**：不会因为某些模型容易犯错就加入额外的教学提示——因为这些"补丁"在帮助笨模型的同时，也会干扰聪明模型的自由发挥，拉低它们的上限。
- **所有战术、伪装、说服、站队，全部由模型自主决策。**

提示词就像一张无偏差的棋盘——棋盘上没有给任何一方设限或提供外挂，下棋水平的高低，完全由模型自己的脑子决定。

> [!CAUTION]
> **不建议修改系统提示词（System Prompt）。** 当前的提示词结构已经过反复验证，对所有水平的模型保持公平中立。任何为了"修复"某个模型表现而添加的补丁或教学指令，都可能在帮助弱模型的同时，无意中约束强模型的发挥空间，破坏评估的公平性。如果某个模型表现不好，那就是它自身能力的真实体现，而不是提示词的问题。
