...
如果您未来计划对 `useGameEngine.ts` 这一上帝状态机 Hook 进行解耦（例如将其抽取为纯函数状态机或独立的 Game Loop 类），建议采用以下**低成本自动化回归测试**方案建立安全防护网：

1. **存档快照作为测试集 (Fixtures)**：将几局正常对局跑完的完整游戏存档（Game Archive JSON，包含逐帧的 GameSnapshot 历史）存入 `fixtures/` 目录。
2. **回归比对测试**：编写一个 Node/TS 测试脚本，自动读取这些 JSON，并在不联网调用 LLM、不运行浏览器 E2E 的情况下，将初始状态与后续事件（如夜间决定、公聊投票）序列喂入新引擎，逐帧比对新引擎算出的新 State 与存档中保存的 Snapshot 帧是否完全相同。
3. **零成本回归**：在确保所有已记录的快照测试断言完全一致后，即可确保状态机的重构没有破坏原有的游戏逻辑，可以安全上线发布。