...
- **没有给笨模型打补丁**：不会因为某些模型容易犯错就加入额外的教学提示。
- **没有给聪明模型拖后腿**：不会用过度约束来限制高级模型的自由发挥。
- **所有战术、伪装、说服、站队，全部由模型自主决策。**

提示词就像一张无偏差的棋盘——棋盘上没有给任何一方设限或提供外挂，下棋水平的高低，完全由模型自己的脑子决定。

> [!CAUTION]
> **不建议修改系统提示词。** 当前的提示词结构已经过反复测试和验证，达到了对所有模型公平且不干预决策的平衡点。任何修改都可能破坏这种公平性——给笨模型加补丁会掩盖它的真实水平，给聪明模型加约束会限制它的发挥上限。如果 AI 在对局中表现不佳，那是模型本身的能力问题，不是提示词的问题。
